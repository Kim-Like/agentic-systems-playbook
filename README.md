...
- Bounded, recency-banded memory — Keep an agent's knowledge sharp: domain-scope new claims, dedupe (token-set similarity + containment), cap the active set with compaction, and band by freshness (fresh / aging / outdated) where age clocks from when the agent LEARNED it. _coming_
...
- Field note: the honest middle on agent hype — Short recurring essays that correct both the overclaim and the dismissal, from a practitioner running this daily. _coming_
